...
pub use gguf_file::{MmapSource, TensorSource};
...
/// A read-only, mmap-backed block store used for zero-copy GGUF tensor
/// loading (see `ggml_file::qtensor_from_mmap` / `gguf_file::MmapSource`).
/// The regular load path copies each tensor's bytes twice - once from the
/// file into a heap `Vec<u8>` in `TensorInfo::read`, again into a `Vec<T>`
/// in `from_raw_data` - which dominates GGUF load time for large models.
/// This instead borrows `data` directly from the memory-mapped file and
/// keeps the mapping alive via `_mmap` for as long as `data` is used.
pub struct MmapedBlocks<T: 'static> {
    // Only kept to extend the mmap's lifetime alongside `data`; never read
    // directly, hence the leading underscore to silence "unused" lints.
    _mmap: std::sync::Arc<memmap2::Mmap>,
    data: &'static [T],
}

impl<T: 'static> MmapedBlocks<T> {
    /// # Safety
    /// `data` must point into `mmap`'s backing memory and `mmap` must
    /// outlive every use of `data`. This is upheld here by storing both in
    /// the same struct (so they're dropped together) and never exposing
    /// `data` on its own.
    pub unsafe fn new(mmap: std::sync::Arc<memmap2::Mmap>, data: &'static [T]) -> Self {
        Self { _mmap: mmap, data }
    }
}

impl<T: k_quants::GgmlType + Send + Sync> QuantizedType for MmapedBlocks<T> {
    fn matmul_t(&self, mkn: (usize, usize, usize), lhs: &[f32], dst: &mut [f32]) -> Result<()> {
        k_quants::matmul(mkn, lhs, self.data, dst)
    }
    ...
    fn from_float(&mut self, _xs: &[f32]) -> Result<()> {
        crate::bail!("cannot write into a read-only mmap-backed quantized tensor")
    }
