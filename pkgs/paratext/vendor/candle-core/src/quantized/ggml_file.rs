...
/// Builds a `QTensor` by borrowing `size_in_bytes` bytes starting at `start`
/// directly from `mmap`, instead of copying them out of a `Read`er first.
/// On `Device::Cpu` this is genuinely zero-copy (see `MmapedBlocks`); on
/// other devices it still avoids the read()/seek() syscalls and the
/// intermediate `Vec<u8>` allocation the `Read`-based path needs, though
/// the host -> device upload copy is unavoidable either way.
fn from_mmap_data<T: super::GgmlType + Send + Sync + 'static>(
    ...
    let n_blocks = size_in_bytes / std::mem::size_of::<T>();
    let raw_data_ptr = mmap[start..start + size_in_bytes].as_ptr();
    let data: QStorage = match device {
        Device::Cpu => {
            // SAFETY: `data` borrows from `mmap`'s backing pages, which have a
            // stable address for the whole mapping's lifetime (memmap2
            // guarantee). We unsafely extend that borrow to `'static` and
            // keep `mmap` alive alongside it inside `MmapedBlocks`, so the
            // memory stays valid for as long as anything can read `data`.
            let data: &'static [T] =
                unsafe { std::slice::from_raw_parts(raw_data_ptr as *const T, n_blocks) };
            QStorage::Cpu(Box::new(unsafe {
                super::MmapedBlocks::new(mmap.clone(), data)
            }))
        }
        Device::Metal(metal) => {
            let data = unsafe { std::slice::from_raw_parts(raw_data_ptr as *const T, n_blocks) };
            super::metal::load_quantized(metal, data)?
        }
        Device::Cuda(cuda) => {
            let data = unsafe { std::slice::from_raw_parts(raw_data_ptr as *const T, n_blocks) };
            super::cuda::load_quantized(cuda, data)?
        }
    };
    super::QTensor::new(data, dims)
}

/// Mmap-backed counterpart to `qtensor_from_ggml`, used for zero-copy GGUF
/// loading (see `gguf_file::MmapSource`).
pub fn qtensor_from_mmap(
    ...
    if start + size_in_bytes > mmap.len() {
        crate::bail!(
            "tensor data (offset {start}, {size_in_bytes} bytes) extends past the end of the mmap'd file ({} bytes)",
            mmap.len()
        )
    }

    match ggml_dtype {
        GgmlDType::F32 => from_mmap_data::<f32>(mmap, start, size_in_bytes, dims, device),
        GgmlDType::F16 => from_mmap_data::<half::f16>(mmap, start, size_in_bytes, dims, device),
        GgmlDType::Q4_0 => {
            from_mmap_data::<k_quants::BlockQ4_0>(mmap, start, size_in_bytes, dims, device)
        }
        GgmlDType::Q4_1 => {
            from_mmap_data::<k_quants::BlockQ4_1>(mmap, start, size_in_bytes, dims, device)
        }
        GgmlDType::Q5_0 => {
            from_mmap_data::<k_quants::BlockQ5_0>(mmap, start, size_in_bytes, dims, device)
        }
        GgmlDType::Q5_1 => {
            from_mmap_data::<k_quants::BlockQ5_1>(mmap, start, size_in_bytes, dims, device)
        }
        GgmlDType::Q8_0 => {
            from_mmap_data::<k_quants::BlockQ8_0>(mmap, start, size_in_bytes, dims, device)
        }
        GgmlDType::Q2K => {
            from_mmap_data::<k_quants::BlockQ2K>(mmap, start, size_in_bytes, dims, device)
        }
        GgmlDType::Q3K => {
            from_mmap_data::<k_quants::BlockQ3K>(mmap, start, size_in_bytes, dims, device)
        }
        GgmlDType::Q4K => {
            from_mmap_data::<k_quants::BlockQ4K>(mmap, start, size_in_bytes, dims, device)
        }
        GgmlDType::Q5K => {
            from_mmap_data::<k_quants::BlockQ5K>(mmap, start, size_in_bytes, dims, device)
        }
        GgmlDType::Q6K => {
            from_mmap_data::<k_quants::BlockQ6K>(mmap, start, size_in_bytes, dims, device)
        }
