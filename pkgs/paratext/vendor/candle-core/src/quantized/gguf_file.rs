...
impl TensorInfo {
    ...
    /// Zero-copy counterpart to `read`: borrows tensor bytes directly from
    /// `mmap` instead of `seek`+`read_exact`-ing them into a fresh buffer.
...
/// Abstraction over where GGUF tensor bytes are read from: either a generic
/// `Read + Seek` stream (the original behaviour, blanket-implemented below
/// so every existing caller keeps working unchanged), or a memory-mapped
/// file (`MmapSource`) for zero-copy CPU loading. `Content::tensor` is
/// generic over this trait instead of `Read + Seek` directly.
...
impl<R: std::io::Read + std::io::Seek> TensorSource for R {
    fn load_tensor(
        &mut self,
        info: &TensorInfo,
        tensor_data_offset: u64,
        ...
        info.read(self, tensor_data_offset, device)
    }
}

/// Zero-copy tensor source backed by a memory-mapped GGUF file. On
/// `Device::Cpu`, tensors borrow directly from the mapped pages with no
/// per-tensor heap copy at all (versus two full copies on the regular
/// `Read`-based path); on other devices this still saves the read()/seek()
/// syscalls and one intermediate allocation, though the unavoidable host ->
/// device upload copy still happens.
pub struct MmapSource(std::sync::Arc<memmap2::Mmap>);

impl MmapSource {
    /// # Safety
    /// Inherited from `memmap2::MmapOptions::map`: the underlying file must
    /// not be modified or truncated while this mapping (or any `QTensor`
    /// built from it) is alive.
    pub unsafe fn open(path: &std::path::Path) -> Result<Self> {
        let file = std::fs::File::open(path).map_err(|e| crate::Error::from(e).with_path(path))?;
        let mmap = memmap2::MmapOptions::new()
            .map(&file)
            .map_err(|e| crate::Error::from(e).with_path(path))?;
        Ok(Self(std::sync::Arc::new(mmap)))
    }
}

impl TensorSource for MmapSource {
    fn load_tensor(
        &mut self,
        info: &TensorInfo,
        tensor_data_offset: u64,
        ...
        info.read_from_mmap(&self.0, tensor_data_offset, device)
    }
}
...
impl Content {
    ...
    pub fn tensor<S: TensorSource>(
        ...
        source.load_tensor(tensor_info, self.tensor_data_offset, device)
    }
}
