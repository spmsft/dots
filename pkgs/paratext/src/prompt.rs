...
pub struct Ops {
    ...
    pub title: bool,
    pub subtitle: bool,
    /// Whether to include the processed "body" key in the output.
    /// Defaults to `true` (see cli.rs's `--body`/`--no-body`) - disabling
    /// it only makes sense alongside at least one of `title`/`subtitle`
    /// (see `validate`).
    pub body: bool,
                ...
                || self.size.is_some())
        {
            bail!(
                "--prompt is mutually exclusive with --summarize/--paraphrase/--passthrough/\
                 --translate/--title/--subtitle/--size (--sanitize/--no-sanitize and --body/\
                 --no-body are default-on/off booleans rather than explicit opt-ins, so they're \
                 not part of this check)"
            );
        }
        if self.custom_prompt.is_none() && !self.title && !self.subtitle && !self.body {
            bail!(
                "at least one of --title/--subtitle/--body must be enabled - \
                 --no-title/--no-subtitle/--no-body were all given (or implied \
                 by defaults), which would produce no output at all"
            );
        }
        Ok(())
    }
...
pub fn build_system_prompt(ops: &Ops) -> (String, Vec<&'static str>) {
    let mut keys = if ops.body { vec!["body"] } else { Vec::new() };
    ...
    if ops.sanitize {
        ...
        instructions.push(
            "Do NOT correct spelling or grammar mistakes - match the input text's own \
             grammar/spelling style as closely as possible, preserving its mistakes rather \
             than fixing them, except where that's unavoidably overridden by an explicit \
             persona/style guide above or by a requested translation below."
                .to_string(),
        );
    }
    ...
    // The closing paragraph spelling out the "body" key's contract only
    // makes sense when --body is actually requested (the default); with
    // --no-body, the instructions above are still applied conceptually
    // (e.g. so a requested --title reflects a summarized/translated
    // version of the text) but no processed body text itself is ever
    // emitted.
    let body_contract = if ops.body {
...
pub fn effective_max_tokens(ops: &Ops, text: &str) -> usize {
    ...
    let mut tokens = if !ops.body {
        // --no-body: no processed body text is ever emitted, so none of
        // the body-length-based estimates below apply - only the
        // title/subtitle overhead added below is relevant.
        0
    } else if ops.custom_prompt.is_some() {
